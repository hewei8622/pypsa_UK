import numpy as np
import pandas as pd
import matplotlib
matplotlib.use('TkAgg')  # Replace with the appropriate backend
import matplotlib.pyplot as plt
from matplotlib.ticker import MaxNLocator



import pypsa
import random
import cartopy.crs as ccrs

# from analysis_functions import cycle_analysis

from mef_log import cycle_analysis


#plot parameters (only plot a selected snapshot for reading purpose)
len_plot=100
idx_start=(random.randint(0, 8760-len_plot))

cmap_t = 'jet'

regions = ['EN_NorthEast','EN_NorthWest','EN_Yorkshire',
           'EN_EastMidlands','EN_WestMidlands',
           'EN_East','EN_London','EN_SouthEast',
           'EN_SouthWest','EN_Wales','Scotland',
           'NorthernIreland']

# regions = ['EN_WestMidlands']

pre_dic = '../results_min_ff/2x_current/'
# pre_dic = 'UK_old_1/'
pre_dic = 'current_2/'

network = pypsa.Network(pre_dic+'network.nc')
df = pd.read_csv(pre_dic+'store_e_carrier_results.csv')
df_capacity = pd.read_csv(pre_dic+'stores_e_nom_opt.csv')
df_gen = pd.read_csv(pre_dic+'gen_p_carrier_results.csv')
df_storage_links = pd.read_csv(pre_dic+'links_p_results.csv')#needs to change to p1 (the pypsa file has been updated)
df_gen_bus = pd.read_csv(pre_dic+'buses_p_results.csv')

df_gen_update = df_gen.copy()
df_gen_update_new = df_gen.copy()

print(network.lines.columns.to_list())
all_carriers = df_gen.columns.to_list()

for region_i in regions:
    region_gen_p = (network.generators_t.p.groupby(network.generators.bus, axis=1).get_group(region_i))
    region_gen_p_carrier =region_gen_p.groupby(network.generators.carrier, axis=1).sum()

    # region_demand = (network.loads_t.p.groupby(network.generators.bus, axis=1).get_group(region_i))
    region_gen_p_carrier['demand'] = network.loads_t.p[region_i]

    # print(region_gen_p)
    file_name = region_i +'_carrier.csv'
    region_gen_p_carrier.to_csv(file_name,header=True)

battery_bus = [s + '_Battery' for s in regions]
ES_bus = [s + '_OtherStorage' for s in regions]
df['soc_batt'] = df['Battery']/df['Battery'].max()
df['soc_ldes'] = df['ES']/df['ES'].max()

battery_charger = [s + '_charger' for s in battery_bus]
battery_discharger = [s + '_discharger' for s in battery_bus]

ES_charger = [s + '_charger' for s in ES_bus]
ES_discharger = [s + '_discharger' for s in ES_bus]


df_storage_links[battery_discharger]=-df_storage_links[battery_discharger]
df_storage_links[ES_discharger]=-df_storage_links[ES_discharger]

# print(df_links_ffmin)
df_storage_links['bus_charger'] = df_storage_links[battery_charger].sum(axis=1)
df_storage_links['bus_discharger'] = df_storage_links[battery_discharger].sum(axis=1)

# print(df_links_ffmin)
df_storage_links['es_bus_charger'] = df_storage_links[ES_charger].sum(axis=1)
df_storage_links['es_bus_discharger'] = df_storage_links[ES_discharger].sum(axis=1)





# fig, ax1 = plt.subplots()

battery_soc=[]
ES_soc=[]

def charge_count(df_soc):
    charg_x = [] #cycle starting idx
    charg_y = [] # cycle ending idx

    zero_point_0 = 0.1 # cycle count starting threshold
    zero_point_1 = .1 # cycle count ending threshold
    zero_con = 1
    for idx, soc_i in enumerate(df_soc):
        if zero_con == 1: #SOC is close to zero
            if soc_i > zero_point_0:
                zero_con = 0 # SOC is larger than zero and start a charge
                charg_x.append(idx)
        if zero_con == 0:
            if soc_i < zero_point_1: # SOC back to zero
                zero_con = 1 # a cycle ends
                charg_y.append(idx)

    print('start points....')
    print('the num of the start points is ' + str(len(charg_x)))
    print(charg_x)

    print('end points')
    print('the num of the end points is ' + str(len(charg_y)))
    print(charg_y)

    charg_xy = np.array(charg_y) - np.array(charg_x) #duration of each cycle (end idx - start idx)

    return charg_x, charg_y,  charg_xy

charg_x, charg_y,  charg_xy = charge_count(df['soc_batt'])


fig, ax = plt.subplots()
# ax.scatter(np.arange(0,len(charg_x),1),charg_xy)
ax.hist(charg_xy, int(len(charg_xy)))
ax.set_title('histogram of battery')

fig, ax_em = plt.subplots()


co2_charge_cycle_new, energy_charge_cycle_new, energy_discharge_cycle_new,emissions_charged, emissions_discharged, df_gen_update_new = cycle_analysis(charg_x, charg_y, df_gen, df_storage_links['bus_charger'],df_storage_links['bus_discharger'], df_gen_update_new,0)
# co2_charge_cycle, energy_charge_cycle, df_gen_update = cycle_analysis(charg_x, charg_y, df_gen, df_storage_links['bus_charger'], df_gen_update)


fig_co2, ax_co2 = plt.subplots(4, 1, figsize=(15, 10), gridspec_kw={'hspace': 0.5})


ax_co2[0].scatter(charg_xy, co2_charge_cycle_new, label='batt')
ax_co2[1].scatter(charg_xy, energy_charge_cycle_new, label='batt')
ax_co2[2].scatter(charg_xy, energy_discharge_cycle_new, label='batt')

scatter_em = ax_em.scatter(emissions_charged, emissions_discharged,c=charg_xy, cmap=cmap_t, label='batt')
ax_em.plot([0, 1e7],[0, 1e7], '--')


# df_gen_update_plot=df_gen_update[idx_start:idx_start+len_plot]
# df_gen_update_plot.plot(y=['Hard coal','Oil','SCGT','CCGT'],ax=ax_gen,linestyle='dashed')

charg_x_es, charg_y_es,  charg_xy_es = charge_count(df['soc_ldes'])


co2_charge_cycle_es, energy_charge_cycle_es, energy_discharge_cycle_es,emissions_charged_es, emissions_discharged_es, df_gen_update = cycle_analysis(charg_x_es, charg_y_es, df_gen_update,df_storage_links['es_bus_charger'],df_storage_links['es_bus_discharger'],df_gen_update,1)
ax_co2[0].scatter(charg_xy_es, co2_charge_cycle_es, label='ldes')
ax_co2[1].scatter(charg_xy_es, energy_charge_cycle_es, label='ldes')
ax_co2[2].scatter(charg_xy_es, energy_discharge_cycle_es, label='ldes')

# print('error',len(charg_xy_es), len(energy_charge_cycle_new))


ax_co2[0].legend()
ax_co2[0].set_title('national battery on top')
ax_co2[0].set_ylabel('CO2 [ton/MWh] per cycle', fontsize=12)
ax_co2[0].set_xscale('log')


ax_co2[1].legend()
ax_co2[1].set_title('national battery on top')
ax_co2[1].set_ylabel('energy charged per cycle [MWh]', fontsize=12)
ax_co2[1].set_xscale('log')
ax_co2[1].set_yscale('log')





# print(df)

df.plot(x='snapshot', y='soc_batt',ax=ax_co2[3])
df.plot(x='snapshot', y='soc_ldes',ax=ax_co2[3])



ax_co2[3].xaxis.set_major_locator(MaxNLocator(6))  # This sets a maximum of 5 x-ticks
ax_co2[3].legend()
ax_co2[3].set_title('national soc')
ax_co2[3].set_xlabel('Time [hour]', fontsize=12)
ax_co2[3].set_ylabel('SOC [MWh]', fontsize=12)

ax_em.scatter(emissions_charged_es, emissions_discharged_es,c=charg_xy_es, cmap=cmap_t, label='ldes')
ax_em.legend()
# ax_co2[4].set_title('emissions of charge and discharge per cycle')
ax_em.set_xlabel('Charge emissions [tCO2]', fontsize=12)
ax_em.set_ylabel('Discharge [tCO2]', fontsize=12)
plt.colorbar(scatter_em, ax=ax_em)

ax_em.set_xscale('log')
ax_em.set_yscale('log')
ax_em.set_xlim(1, 1e7)
ax_em.set_ylim(1, 1e7)


fig_bus_batt, ax_co2_bus = plt.subplots(2, 1, figsize=(12, 5), gridspec_kw={'hspace': 0.5})



df_store = (network.stores_t.e)
stores = ['Battery', 'OtherStorage']

storage_batt = []
storage_ldes = []


for i in regions:
    fig, ax = plt.subplots(5, 1, figsize=(15, 10), gridspec_kw={'hspace': 0.5})
    for j in stores:
        store_carrier = i + '_' + j
        df_store.plot(y=store_carrier, ax=ax[0])
        ax[0].set_title('Energy storage SOC in MWh')
        ax[0].set_ylabel('SOC [MWh]')

        if j == 'Battery':
            storage_batt.append(df_store[store_carrier].max())
        else:
            storage_ldes.append(df_store[store_carrier].max())


    battery_i = i+'_Battery'
    df_capacity[battery_i + '_soc'] = df_capacity[battery_i]/df_capacity[battery_i].max()
    battery_soc.append(battery_i + '_soc')

    ES_i = i + '_OtherStorage'
    df_capacity[ES_i + '_soc'] = df_capacity[ES_i] / df_capacity[ES_i].max()
    ES_soc.append(ES_i + '_soc')


    file_name = i + '_carrier.csv'
    df_gen_bus_carrier = pd.read_csv(file_name)

    # df_gen_bus_carrier.plot(y=df_gen_bus_carrier.columns.to_list(), ax=ax[2])

    df_gen_bus_carrier = df_gen_bus_carrier.reindex(columns=all_carriers, fill_value=0)
    df_gen_bus_carrier_update = df_gen_bus_carrier

    print(i)
    # print('coal',df_gen_bus_carrier['Hard coal'].sum())
    # print('oil', df_gen_bus_carrier['Oil'].sum())
    # print('SCGT', df_gen_bus_carrier['SCGT'].sum())
    # print('CCGT', df_gen_bus_carrier['CCGT'].sum())

    for cols in df_gen_bus_carrier.columns[1:]:
        print(cols, df_gen_bus_carrier[cols].sum())

    charg_x, charg_y,  charg_xy = charge_count(df_capacity[battery_i + '_soc'])
    co2_charge_cycle, energy_charge_cycle, energy_discharge_cycle, emissions_charged, emissions_discharged,df_gen_bus_carrier_update = cycle_analysis(charg_x, charg_y, df_gen_bus_carrier, df_storage_links[battery_i + '_charger'],df_storage_links[battery_i + '_discharger'], df_gen_bus_carrier_update,0)
    ax_co2_bus[0].scatter(charg_xy, co2_charge_cycle, label=i)





    ax[1].scatter(charg_xy, co2_charge_cycle, label='battery')
    ax[2].scatter(charg_xy, energy_charge_cycle, label='battery')
    ax[3].scatter(charg_xy, energy_discharge_cycle, label='battery')
    scatter_4 = ax[4].scatter(emissions_charged, emissions_discharged, c=charg_xy, cmap=cmap_t, label='battery')
    # print('Batt emissons: ', emissions_charged, emissions_discharged)
    print('Batt cycle eff', np.array(energy_discharge_cycle) / np.array(energy_charge_cycle))



    # df_storage_links.plot(y= [battery_i + '_charger',battery_i + '_discharger'], ax=ax[2], label=['batt_c','batt_d'])



    charg_x_es, charg_y_es, charg_xy_es = charge_count(df_capacity[ES_i + '_soc'])
    co2_charge_cycle_es, energy_charge_cycle_es, energy_discharge_cycle_es,emissions_charged_es, emissions_discharged_es, df_gen_bus_carrier_update = cycle_analysis(charg_x_es, charg_y_es,
                                                           df_gen_bus_carrier_update,
                                                           df_storage_links[ES_i + '_charger'],df_storage_links[ES_i + '_discharger'], df_gen_bus_carrier_update,1)
    # print('LDES emissons: ', emissions_charged_es, emissions_discharged_es)
    print('LDES cycle eff', np.array(energy_discharge_cycle_es)/np.array(energy_charge_cycle_es))


    ax_co2_bus[1].scatter(charg_xy_es, co2_charge_cycle_es, marker='s', label=i)

    # print(i)
    # print(charg_xy_es, co2_charge_cycle_es)



    ax[1].scatter(charg_xy_es, co2_charge_cycle_es, marker='s', label='ldes')
    ax[2].scatter(charg_xy_es, energy_charge_cycle_es, marker='s', label='ldes')
    ax[3].scatter(charg_xy_es, energy_discharge_cycle_es, marker='s', label='ldes')
    ax[4].scatter(emissions_charged_es, emissions_discharged_es,c=charg_xy_es, cmap=cmap_t, marker='s', label='ldes')
    ax[4].plot([0, 1e7],[0,1e7],'--')

    # df_storage_links.plot(y=[ES_i + '_charger', ES_i + '_discharger'], ax=ax[2],label=['ldes_c','ldes_d'])

    ax[1].set_xlabel('Storage Duration [hour]')
    ax[1].set_ylabel('CO2 Emissions [ton/MWh]')
    ax[1].set_xscale('log')

    ax[2].set_xlabel('Storage Duration [hour]')
    ax[2].set_ylabel('Energy charged [MWh]')
    ax[2].set_xscale('log')
    ax[2].set_yscale('log')

    ax[3].set_xlabel('Storage Duration [hour]')
    ax[3].set_ylabel('Energy discharged [MWh]')
    ax[3].set_xscale('log')
    ax[3].set_yscale('log')

    ax[4].set_xlabel('Charged emissions [tCO2]')
    ax[4].set_ylabel('Discharged emissions [tCO2]')
    ax[4].set_xscale('log')
    ax[4].set_yscale('log')
    ax[4].set_xlim(1, 1e7)
    ax[4].set_ylim(1, 1e7)

    plt.colorbar(scatter_4, ax=ax[4])

    # ax_co2_bus[0].set_xscale('log')
    ax_co2_bus[0].set_xlim(0, 8760)  # Adjust min and max as needed
    # ax_co2_bus[0].set_ylim(0, 1)  # Adjust min and max as needed

    ax_co2_bus[0].set_xscale('log')
    ax_co2_bus[0].set_title('all region results - Batt')
    ax_co2_bus[0].legend()
    ax_co2_bus[0].set_title('battery')
    ax_co2_bus[0].set_xlabel('Storage Duration [hour]')
    ax_co2_bus[0].set_ylabel('CO2 Emissions [ton/MWh]')

    ax_co2_bus[1].set_title('all region results - LDES')
    ax_co2_bus[1].set_xlabel('Storage Duration [hour]')
    ax_co2_bus[1].set_ylabel('CO2 Emissions [ton/MWh]')
    ax_co2_bus[1].set_xlim(0, 8760)  # Adjust min and max as needed
    # ax_co2_bus[1].set_ylim(0, 1)  # Adjust min and max as needed
    ax_co2_bus[1].set_xscale('log')




    # ###
    # file_name = i + '_carrier.csv'
    # df_gen_bus_carrier = pd.read_csv(file_name)
    #
    #
    # df_gen_bus_carrier = df_gen_bus_carrier.reindex(columns=all_carriers, fill_value=0)
    # df_gen_bus_carrier_update = df_gen_bus_carrier
    #
    # charg_x, charg_y, charg_xy = charge_count(df_capacity[ES_i + '_soc'])
    # co2_charge_cycle, energy_charge_cycle, df_gen_bus_carrier_update = cycle_analysis_new(charg_x, charg_y,
    #                                                                                   df_gen_bus_carrier,
    #                                                                                   df_storage_links[
    #                                                                                       ES_i + '_charger'],
    #                                                                                   df_gen_bus_carrier_update)
    #
    # ax[2].scatter(charg_xy, co2_charge_cycle, marker='s', label='ldes')
    # ax[1].scatter(charg_xy, co2_charge_cycle, facecolor='none')
    # # ax[2].set_title('LDES CO2 Emissions')
    # ax[2].set_xlabel('Storage duration [hour]')
    # ax[2].set_ylabel('CO2 Emissions [ton/MWh]')
    #
    # # df_storage_links.plot(y= [battery_i + '_charger',battery_i + '_discharger'], ax=ax[2], label=['batt_c','batt_d'])
    #
    # charg_x_es, charg_y_es, charg_xy_es = charge_count(df_capacity[battery_i + '_soc'])
    # co2_charge_cycle_es, energy_charge_cycle_es, df_gen_bus_carrier_update = cycle_analysis_new(charg_x_es, charg_y_es,
    #                                                                                         df_gen_bus_carrier_update,
    #                                                                                         df_storage_links[
    #                                                                                             battery_i + '_charger'],
    #                                                                                         df_gen_bus_carrier_update)
    #
    # ax[2].scatter(charg_xy_es, co2_charge_cycle_es, label='battery')
    # ax[1].scatter(charg_xy_es, co2_charge_cycle_es, facecolor='none')







plt.show()

